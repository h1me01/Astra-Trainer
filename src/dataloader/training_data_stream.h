...
struct BinSfenInputStream : BasicSfenInputStream {
    ...
    BinSfenInputStream(std::string filename, bool cyclic, std::function<bool(const TrainingDataEntry &)> skipPredicate)
        : m_stream(filename, openmode), m_filename(filename), m_eof(!m_stream), m_cyclic(cyclic),
...
struct BinpackSfenInputStream : BasicSfenInputStream {
    ...
    BinpackSfenInputStream(std::string filename, bool cyclic, std::function<bool(const TrainingDataEntry &)> skipPredicate)
        : m_stream(std::make_unique<binpack::CompressedTrainingDataEntryReader>(filename, openmode)),
          m_filename(filename), m_eof(!m_stream->hasNext()), m_cyclic(cyclic),
          m_skipPredicate(std::move(skipPredicate)) {}
...
struct BinpackSfenInputParallelStream : BasicSfenInputStream {
    ...
    BinpackSfenInputParallelStream(int concurrency,
        ...
        : m_stream(std::make_unique<binpack::CompressedTrainingDataEntryParallelReader>(
              concurrency, filenames, openmode, cyclic, skipPredicate)),
          m_filenames(filenames), m_concurrency(concurrency), m_eof(false), m_cyclic(cyclic),
...
inline std::unique_ptr<BasicSfenInputStream> open_sfen_input_file(
    const std::string &filename, bool cyclic, std::function<bool(const TrainingDataEntry &)> skipPredicate = nullptr) {
