...
get_radam_update(float mom, float vel, float lr, float eps, float beta1_t, float beta2_t, int N_sma, int N_sma_max) {
    if (N_sma >= 5) {
        const float step_size =
            lr *
            sqrtf(
                (1.0f - beta2_t) * (N_sma - 4) / (N_sma_max - 4) * (N_sma - 2) / N_sma * N_sma_max / (N_sma_max - 2)
            ) /
            (1.0f - beta1_t);
