...
__global__ void pairwise_mul_fwd_kernel(
    const float* in_v, float* out_v, const int feature_size, const int out_r, const int batch_size
) {
    ...
    out_v[out_idx] = activate_fwd<act_type>(val);
}
...
void pairwise_mul_fwd(const DenseMatrix& in_v, DenseMatrix& out_v, const int out_offset, const Activation act_type) {
    ...
    DISPATCH_ACTIVATION(
        ...
        <<<blocks, block_size>>>(
            in_v.dev_address(), out_v.dev_address() + out_offset, feature_size, out_v.rows(), in_v.cols()
        )
    );
}
